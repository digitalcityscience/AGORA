    ...
    <Card class="w-full isochrone-filter">
        ...
        <template #content>
            ...
            <div class="w-full 2xl:flex 2xl:justify-between 2xl:grid-cols-none lg:grid lg:grid-cols-4 lg:gap-2 2xl:gap-0 p-1 ">
                <div class="">
                    <Button v-if="!selectionOnProgress" size="small" class="text-xs leading-4 2xl:grow-0 lg:w-full"
                        @click="startCenterSelection">
                        <template #icon>
                            <i class="material-icons">add_location_alt</i>
                        </template>
                    </Button>
                    <Button v-else size="small" class="text-xs leading-4 2xl:grow-0" @click="cancelCenterSelection">
                        <template #icon>
                ...
                <div class="lg:grid lg:grid-cols-subgrid lg:col-span-3 2xl:grid-cols-none">
                    <SelectButton class="2xl:grow-0 lg:col-span-3 travel-mode flex" v-model="selectedTravelMode" :options="traveModeList"
                ...
                <div class="lg:grid lg:grid-cols-subgrid lg:col-span-4 2xl:grid-cols-none">
                    <InputNumber v-model="travelTime" input-class="grow lg:col-span-4" suffix="min" :min="0"></InputNumber>
                </div>
                <div class="lg:grid lg:grid-cols-subgrid lg:col-span-4 2xl:grid-cols-none">
                    <Button class="lg:col-span-4 2xl:grow-0" :disabled="centerPoint === undefined" @click="createIsochrone">Create</Button>
                </div>
            </div>
            <div class="w-full grid grid-cols-4 pt-1" v-if="isochroneOnTheMap && isochroneOnTheMapData">
                <div class="p-1 lg:col-span-2">
                    <Button class="w-full" size="small" @click="addSelectedIsochrone(isochroneOnTheMapData)">Add to
                        list</Button>
                </div>
                <div class="p-1 lg:col-span-2">
